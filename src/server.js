...
// Integration test endpoint — runs inside the container where all tools and env vars are present.
// CI calls this via curl; returns 200 + JSON on full pass, 500 on any failure.
app.get("/integration-test", async (_req, res) => {
  const results = {};
  let allOk = true;

  // Himalaya — installed in the Docker image
  ...
  // Telegram — optional; only fails if token is set but invalid
  ...
  if (!telegramToken) {
    ...
    try {
      ...
      if (data.ok) {
        results.telegram = { ok: true, detail: data.result.first_name };
      } else {
  ...
  if (!anthropicKey) {
    ...
    try {
      ...
      const data = await r.json();
      if (data.content) {
        results.claude_api = { ok: true };
      } else {
        results.claude_api = { ok: false, detail: data.error?.message || "unexpected response" };
        allOk = false;
      }
    } catch (err) {
  ...
  // GitHub — optional; only fails if token is set but invalid
  ...
  if (!githubPat) {
    ...
    try {
      ...
      if (data.login) {
        ...
        results.github_api = { ok: false, detail: data.message || "invalid token" };
        allOk = false;
      }
    } catch (err) {
